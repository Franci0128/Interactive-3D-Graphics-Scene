...
namespace gps {
            ...
            if (shapes[s].mesh.material_ids.size() > 0 && materials.size() > 0) {
                ...
                if (materialId != -1) {
                    ...
                    if (!diffuseTexturePath.empty()) {
                        textures.push_back(LoadTexture(basePath + diffuseTexturePath, "diffuseTexture"));
                    }
                    // Specular texture
                    std::string specularTexturePath = materials[materialId].specular_texname;
                    if (!specularTexturePath.empty()) {
                        textures.push_back(LoadTexture(basePath + specularTexturePath, "specularTexture"));
                    }
                }
            }
    ...
    gps::Texture Model3D::LoadTexture(std::string path, std::string type) {
        for (int i = 0; i < loadedTextures.size(); i++) {
            if (loadedTextures[i].path == path) return loadedTextures[i];
        }
    ...
    GLuint Model3D::ReadTextureFromFile(const char* file_name) {
        ...
        // Vertical flip (OpenGL expects UVs from bottom-left)
        ...
        // MODIFICARE CRITICĂ: GL_SRGB_ALPHA permite canalul de transparență
        ...
        // MODIFICARE: Eliberăm memoria RAM după ce am urcat textura pe GPU
